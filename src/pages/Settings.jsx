...
import { getWalletSeed } from '../services/api'; // Make sure this is in your api.js!
import { 
...
// Unified Hover Tooltip for consistency
...
export default function Settings({ user, onLogout }) {
  ...
  // Hardcoded Nostr Key (Ideally this would come from the API too)
  ...
  const handleRevealSeed = async () => {
    ...
    try {
      // LIVE API CALL
      const data = await getWalletSeed(activePhone);
      
      if (data && data.seed_phrase) {
        // If the backend sends a string, split it into an array
        const phraseArray = typeof data.seed_phrase === 'string' 
          ? data.seed_phrase.split(' ') 
          : data.seed_phrase;
      ...
      setSeedError("Could not retrieve seed from server. Showing local backup.");
      // Fallback dummy phrase for hackathon demo resilience
      setSeedPhrase(["breeze", "dune", "pioneer", "execute", "absorb", "clerk", "various", "custom", "vintage", "satoshi", "lion", "dilemma"]);
    } finally {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-start">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <section className={`p-6 rounded-[2rem] border transition-all duration-500 hover:shadow-xl hover:-translate-y-1 ${isDarkMode ? 'bg-[#0a0a0a] border-blue-900/30 hover:border-blue-700/50 hover:shadow-black/50' : 'bg-white border-blue-100 hover:border-blue-200 hover:shadow-blue-900/5'}`}>
              ...
              <HoverTooltip text={copied ? "Copied!" : "Copy to clipboard"}>
                <button 
                  onClick={handleCopy}
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            {/* Wallet Recovery Section - NOW WIRED UP */}
            <section className={`p-6 rounded-[2rem] border transition-all duration-500 hover:shadow-xl hover:-translate-y-1 ${isDarkMode ? 'bg-[#0a0a0a] border-blue-900/30 hover:border-blue-700/50 hover:shadow-black/50' : 'bg-white border-blue-100 hover:border-blue-200 hover:shadow-blue-900/5'}`}>
              ...
              {!showRecovery ? (
                ...
                <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                  ...
                  {seedError && (
                    <p className="text-xs font-bold text-orange-500 mb-3 text-center">{seedError}</p>
                  )}
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 sm:gap-3 mb-4">
                    {isFetchingSeed ? (
                      ...
                      Array(12).fill(0).map((_, i) => (
                        <div key={i} className={`h-10 rounded-lg animate-pulse ${isDarkMode ? 'bg-zinc-800' : 'bg-slate-200'}`}></div>
                      ))
                    ) : (
                      // Actual Seed Words
                  ...
                  <div className="flex gap-3">
                    <button 
                      ...
                      className={`flex-1 py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all duration-300 disabled:opacity-50 ${isDarkMode ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-500/20'}`}
                    >
                    ...
                    <button 
                      onClick={() => {
                        ...
                        setSeedPhrase([]); // Clear for security
                      }}
